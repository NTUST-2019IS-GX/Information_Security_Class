...
class DESMethod:
    def __init__(self, key, text):
        ...
        self.keybinary = hex2bin(self.key)
        self.textbinary = hex2bin(self.text)

        # Generate 16 Rounds Keys
        self.key_list = []

        pc_1_key = transform(self._pc1, self.keybinary)
        c = pc_1_key[:28]
        d = pc_1_key[28:]

        for i in range(16):
            c = shift(c, self._shift[i])
